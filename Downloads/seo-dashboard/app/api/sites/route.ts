...
export async function GET(req: NextRequest) {
  ...
  const apiKey = searchParams.get("key") || process.env.WINDSOR_API_KEY;
  ...
  try {
    ...
    const res = await fetch(`https://connectors.windsor.ai/searchconsole?${params}`, {
      next: { revalidate: 3600 },
    });
    if (!res.ok) {
      const t = await res.text();
      throw new Error(`Windsor error ${res.status}: ${t.slice(0, 200)}`);
    }
